...
namespace Drupal\po_stock\Service;
...
class StockHelper {
  ...
  private function alterCartForm(array &$form) {
    ...
    $form['#validate'] = array_merge($form['#validate'], [
      '\Drupal\po_stock\Service\StockHelper::validateAddToCart',
    ]);
  ...
  private function alterCheckout(array &$form) {
    ...
    $form['#validate'] = array_merge($form['#validate'], [
      '\Drupal\po_stock\Service\StockHelper::validateCheckout',
    ]);
  }
  ...
  public static function validateAddToCart(
    ...
    /** @var \Drupal\po_stock\Service\StockValidation $stockValidation */
    $stockValidation = \Drupal::service('po_stock.stock_validation');
   ...
   */
  private static function validateCheckout(
    array $form,
    FormStateInterface $form_state
  ) {
      ...
      if ($stockValidation->checkProductStock($purchased_entity, $qty)) {
        $form_state->setError(
          ...
          t("The maximum quantity for %name that can be ordered is %qty.", [
            '%name' => $name,
            '%qty' => $purchased_entity->get('field_po_stock')->value,
          ])
        );
      }
    }
