...
namespace Drupal\po_stock\EventSubscriber;
...
class Order implements EventSubscriberInterface {
  ...
  private function modifyStockLevel(
    ...
    $stock = $productVariation->get('field_po_stock')->value + $quantity;
    $productVariation->set('field_po_stock', $stock);
  ...
  public function onOrderCancel(WorkflowTransitionEvent $event) {
    ...
    foreach ($order->getItems() as $item) {
      ...
      $stock = $entity->get('field_po_stock')->value + $quantity;
      $entity->set('field_po_stock', $stock);
  ...
  public function onOrderItemUpdate(OrderItemEvent $event) {
    ...
    if ($order && !in_array($order->getState()->value, ['draft', 'canceled'])) {
      ...
      if ($diff) {
        ...
        $stock = $entity->get('field_po_stock')->value + $diff;
        $entity->set('field_po_stock', $stock);
  ...
  public function onOrderItemDelete(OrderItemEvent $event) {
    ...
    if ($order && !in_array($order->getState()->value, ['draft', 'canceled'])) {
      ...
      $stock = $entity->get('field_po_stock')->value + $item->getQuantity();
      $entity->set('field_po_stock', $stock);
