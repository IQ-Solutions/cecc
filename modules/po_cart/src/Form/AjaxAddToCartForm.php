...
use Drupal\Core\Url;
use Drupal\po_cart\Helper\RefreshPageElements;
...
class AjaxAddToCartForm extends AddToCartForm implements AddToCartFormInterface {
   ...
   *
   * @var \Drupal\po_cart\Helper\RefreshPageElements
   */
   ...
   *   The current user.
   * @param \Drupal\po_cart\Helper\RefreshPageElements $refresh_page_elements_helper
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('po_cart.refresh_page_elements_helper'),
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $instock = $this->stockValidation->checkProductStock($purchasedEntity);
    ...
    if (!$purchasedEntity->get('field_maximum_order_amount')->isEmpty() && $instock) {
      $form['order_limit_display'] = [
        '#type' => 'container',
      ];

      $form['order_limit_display']['order_limit'] = [
        '#type' => 'item',
        '#markup' => $this->t('Limit: %orderLimit', [
          '%orderLimit' => $purchasedEntity->get('field_maximum_order_amount')->value,
        ]),
      ];

      $form['quantity']['widget'][0]['value']['#max'] = $purchasedEntity->get('field_maximum_order_amount')->value;
    }
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    if ($addToCartType == 'cart') {
      $buttonText = $this->stockValidation->getOrderedQuantity($purchasedEntity) > 0 ?
      $this->t('Update') : $this->t('Add');
    }
