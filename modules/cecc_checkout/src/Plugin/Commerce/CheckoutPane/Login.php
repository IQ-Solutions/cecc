 ...
 *   id = "cecc_login",
 *   label = @Translation("Login or continue as guest - no username"),
 *   default_step = "login",
 * )
 */
class Login extends CheckoutPaneBase implements CheckoutPaneInterface, ContainerFactoryPluginInterface {
  ...
  public function validatePaneForm(array &$pane_form, FormStateInterface $form_state, array &$complete_form) {
    ...
    switch ($trigger) {
      ...
      case 'register':
        $username = Xss::filter($values['register']['name']);
        $password = Xss::filter(trim($values['register']['password']));
        if (empty($username)) {
          $form_state->setError($pane_form['register']['name'], $this->t('Email field is required.'));
          return;
        }
        if (empty($password)) {
        ...
        foreach ($violations->getByFields(['name', 'pass', 'mail']) as $violation) {
          ...
          $field_name = $field_name == 'name' || $field_name == 'mail' ? 'name' : 'pass';
